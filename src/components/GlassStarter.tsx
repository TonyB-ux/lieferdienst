...
const ALL_CATEGORIES = ["Gemüsekiste","Obstkiste","Abo","Vegan","Familie","Büro-Obst","Regional"];
...
export default function GlassStarter() {
  ...
  const [country, setCountry] = useState<"ALL" | "DE" | "AT" | "CH">("ALL");
    ...
    <div className="min-h-screen text-white bg-slate-900">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 md:px-6 lg:px-8">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  {[{ label: "Alle", val: "ALL" }, { label: "DE", val: "DE" }, { label: "AT", val: "AT" }, { label: "CH", val: "CH" }].map((opt) => (
                    <Chip key={opt.val} active={country === (opt.val as any)} onClick={() => setCountry(opt.val as any)}>
                      {opt.label}
                    </Chip>
                  ))}
                </div>
        ...
        <section className="mt-6">
          ...
          {results.length === 0 && (
            <GlassCard className="p-8 mt-6 text-center text-white/80">Keine Ergebnisse. Passe deine Filter an.</GlassCard>
          )}
        </section>
    ...
    <section className="relative overflow-hidden">
      <div className="mx-auto max-w-5xl px-4 md:px-6 lg:px-8 py-10 md:py-16">
        <GlassCard className="p-6 md:p-10">
          ...
          <p className="mt-3 text-white/80 max-w-2xl">Neutraler Überblick für Deutschland, Österreich und die Schweiz – mit direktem Shop-Link.</p>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-white/80">{supplier.city} • {supplier.country}</p>
          </div>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {supplier.badges.map((b: string) => (<Badge key={b}>{b}</Badge>))}
        </div>
        <div className="mt-4 flex items-center justify-between">
          <div className="flex gap-1 text-xs text-white/70">
            {supplier.categories.map((c: string) => (
              <span key={c} className="rounded-md px-2 py-1 bg-white/10 border border-white/20">{c}</span>
            ))}
          </div>
          <a href={supplier.url} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 rounded-lg bg-white/90 text-slate-900 text-sm hover:bg-white">
            Zum Webshop
          </a>
        </div>
      </div>
    ...
    <GlassCard className="p-6 md:p-8" id="newsletter">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <form onSubmit={(e)=>e.preventDefault()} className="flex gap-2 w-full md:w-auto">
          <input type="email" required placeholder="E-Mail" className="flex-1 md:w-72 rounded-xl bg-white/15 border border-white/30 px-4 py-3 placeholder-white/60 text-white focus:outline-none" />
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6 lg:px-8 py-10 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          ...
          <ul className="space-y-1 text-sm text-white/80">
            {ALL_CATEGORIES.slice(0,5).map((c) => (<li key={c}><a href="#" className="hover:text-white">{c}</a></li>))}
          </ul>
        </div>
        <div>
          <h4 className="font-semibold mb-2">Regionen</h4>
          <ul className="space-y-1 text-sm text-white/80">
            {["Berlin","München","Hamburg","Köln","Zürich","Wien"].map((r) => (<li key={r}><a href="#" className="hover:text-white">{r}</a></li>))}
          </ul>
        </div>
        <div>
